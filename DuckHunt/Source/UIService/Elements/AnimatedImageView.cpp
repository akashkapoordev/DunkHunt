...
namespace UI
{
	namespace UIElement
	{
		...
		{
			float alpha = std::min(1.0f, elapsed_duration / animation_duration); // we don't want the opacity to be more than '1'.
			imageSprite.setColor(sf::Color(255, 255, 255, static_cast<sf::Uint8>(alpha * 255)));
		}
		void AnimatedImageView::fadeOut()
		{
			float alpha = std::max(0.0f, 1.0f - (elapsed_duration / animation_duration)); //don't want the opacity to go below '0' - > we decrease the aplha from 1 to 0.
			imageSprite.setColor(sf::Color(255, 255, 255, static_cast<sf::Uint8>(alpha * 255)));
		}
